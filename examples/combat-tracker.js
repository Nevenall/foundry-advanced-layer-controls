...
class CombatTracker extends SidebarTab {
   ...
   initialize({
      ...
      if (this._popout) this._popout.initialize({
         combat,
         render: false
      });
   ...
   async getData(options) {
      ...
      for (let [i, t] of combat.turns.entries()) {
         ...
         if (VideoHelper.hasVideoExtension(t.img)) {
            ...
            else t.img = t.thumb = await game.video.createThumbnail(t.img, {
               width: 100,
               height: 100
            });
         }
   ...
   activateListeners(html) {
      ...
      const observer = new IntersectionObserver(this._onLazyLoadImage.bind(this), {
         root: tracker[0]
      });
   ...
   async _onCombatCreate(event) {
      ...
      if (!scene) return;
      let cbt = await game.combats.object.create({
         scene: scene._id
      });
      await cbt.activate();
      this.initialize(cbt);
